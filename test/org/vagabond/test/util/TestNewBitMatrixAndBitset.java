...
public class TestNewBitMatrixAndBitset {
	static Logger log = Logger.getLogger(TestNewBitMatrixAndBitset.class);
	...
	private void checkSet(int[] bits, NewEWAHBitmap set) {
		for (int i = 0; i < bits.length; i++) {
			assertTrue("i: " + bits[i] + "\n" + set.bufferToString(),
					set.intersects(NewEWAHBitmap.getSingleton(bits[i])));
	...
	private void assertEqualsBitset(NewEWAHBitmap set1,
			NewEWAHBitmap set2, String name) {
		assertEquals(
	...
	public void testBitsetGet() {
		NewEWAHBitmap bitset = new NewEWAHBitmap(), bitset2;
		setBits(setBits1, bitset);

		checkSet(setBits1, bitset);

		for (int i = 0; i < 100; i++) {
			bitset2 = new NewEWAHBitmap();
	...
	public void testBitSetToBitString() {
		NewEWAHBitmap bitset = new NewEWAHBitmap();
	...
	public void testBitSetFromString() {
		...
		NewEWAHBitmap bitset = new NewEWAHBitmap(value);
		...
		bitset = new NewEWAHBitmap(value);
		assertEquals(value, bitset.toBitsString());
	}
	...
	public void testBitsetSaveSet() {
		NewEWAHBitmap bitset = new NewEWAHBitmap();
		NewEWAHBitmap bitset2 = new NewEWAHBitmap();
		setBits(setBits1, bitset);
	...
	public void testBitsetClone() {
		NewEWAHBitmap bitset = new NewEWAHBitmap();
		NewEWAHBitmap bitset2;

		setBits(new int[] { 121, 0 }, bitset);
		bitset2 = (NewEWAHBitmap) bitset.clone();
	...
	public void testLogicalOperations() {
		NewEWAHBitmap b1, b2, ex, res;

		// OR
		b1 = new NewEWAHBitmap("100");
		b2 = new NewEWAHBitmap("001");
		ex = new NewEWAHBitmap("101");
		res = (NewEWAHBitmap) b1.or(b2);
		assertEquals(ex.toBitsString() + " - " + res.toBitsString(), ex, res);

		// AND
		b1 = new NewEWAHBitmap("101");
		b2 = new NewEWAHBitmap("011");
		ex = new NewEWAHBitmap("001");
		res = (NewEWAHBitmap) b1.and(b2);
		assertEquals(ex.toBitsString() + " - " + res.toBitsString(), ex, res);

		// AND NOT
		b1 = new NewEWAHBitmap("110");
		b2 = new NewEWAHBitmap("011");
		ex = new NewEWAHBitmap("100");
		ex.setSizeInBits(3);
		res = (NewEWAHBitmap) b1.andNot(b2);
		assertEquals(ex.toBitsString() + " - " + res.toBitsString(), ex, res);
		...
		b2 = new NewEWAHBitmap("011");
		ex = new NewEWAHBitmap("101");
		res = (NewEWAHBitmap) b1.xor(b2);
		assertEquals(ex.toBitsString() + " - " + res.toBitsString(), ex, res);

		// NOT
		b1 = new NewEWAHBitmap("101");
		b1.not();
		ex = new NewEWAHBitmap("010");
		ex.setSizeInBits(3);
